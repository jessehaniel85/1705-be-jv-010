# Material do Aluno — Aula 4: Comunicação assíncrona (producer/consumer)
...
O ganho que justifica toda a complexidade é o **desacoplamento temporal**: produtor e consumidor **não precisam estar vivos ao mesmo tempo**. Compare com a chamada síncrona:

| Dimensão | Síncrono (HTTP direto) | Assíncrono (fila) |
|---|---|---|
| Disponibilidade | produtor e consumidor caem juntos | consumidor pode estar fora; mensagens esperam |
| Latência vista pelo cliente | soma de todo o trabalho | só a validação + publicação |
| Absorção de pico | nenhuma (rejeita ou enfileira na thread) | a fila é o buffer natural |
| Escala do consumidor | acoplada à do produtor | independente (N consumidores) |
| Acoplamento de localização | produtor conhece o endereço do consumidor | produtor conhece só a exchange |
| Complexidade operacional | baixa | alta (broker, duplicação, ordem, DLQ) |

Repare na última linha: o assíncrono **não é grátis**. Ele troca simplicidade por disponibilidade e elasticidade. A última seção desta aula é justamente sobre **quando não fazer isso**.

---

## 3. Mensagem, comando e evento: três coisas diferentes

"Mensagem" é o termo guarda-chuva: qualquer pacote de dados que trafega pelo broker. Dentro dele, há duas intenções **semanticamente opostas**, e confundi-las é uma das causas mais comuns de arquitetura distribuída que vira espaguete.

- **Comando:** uma instrução **dirigida** a um consumidor específico, com intenção de **ação**. Tempo verbal imperativo: *"Grave este comprovante"* → `GravarComprovanteCommand`. Um comando tem **um** dono lógico (quem deve executá-lo) e o produtor **espera** que algo aconteça como resultado. O produtor sabe que pediu uma ação.
- **Evento:** a notificação de um **fato consumado**, no passado, que **qualquer interessado** pode observar. Tempo verbal no particípio: *"O comprovante foi gravado"* → `ComprovanteGravadoEvent`. O produtor do evento **não sabe nem se importa** com quem vai reagir. Pode ser zero, um ou dez consumidores (notificação, antifraude, BI).
...
| Exemplo no PIX | `GravarComprovanteCommand` (Aula 4/5) | `ComprovanteGravadoEvent` (Aula 6) |

**Por que a distinção é de projeto, não de detalhe?** Porque ela define **a direção do acoplamento**. Quando o produtor de um "evento" começa a esperar uma ação específica de um consumidor específico — "publiquei `ComprovanteGravado` e *preciso* que o serviço de notificação faça X" — esse evento virou um **comando disfarçado**, e você recriou o acoplamento que a fila deveria ter quebrado. A regra prática:

> Se você consegue adicionar um novo consumidor sem alterar o produtor, é um evento. Se o produtor precisaria saber que o consumidor existe, é um comando.

No projeto PIX, a gravação é um **comando** (a emissão *pede* que se grave; há um responsável claro). Já o "comprovante foi gravado" é um **evento** (a gravação anuncia um fato; quem reage é problema de quem reage). Esta aula trata do comando; a Aula 6 trata do evento.
...
2. **Limitar o que cada consumidor puxa (prefetch/QoS):** evita que um consumidor "engula" milhares de mensagens e estoure a própria memória (detalhe da Aula 5).
...
O emissor **gera o id**, valida de forma síncrona e publica numa exchange (a anatomia da exchange é a Aula 5; aqui foco no padrão produtor):
...
> **Na próxima aula:** o consumidor desta aula tem um buraco. Quando a gravação falha por **azar momentâneo** (banco oscilou), reprocessar resolve. Mas quando a mensagem está **malformada** e vai falhar **sempre**, reprocessar para sempre trava a fila inteira — é a *poison message*. Como distinguir os dois tipos de falha, reprocessar um com **retry e backoff** e isolar o outro numa **Dead Letter Queue** sem nunca perder o dado? É a entrada do RabbitMQ de verdade: exchanges, bindings, ack manual, prefetch e DLQ.
